...
// Redare automată la încărcarea paginii
window.addEventListener('load', () => {
    audio.play().catch(error => {
        console.log('Redarea automată a fost blocată:', error);
    });
    playerButton.innerHTML = pauseIcon; // Setezi iconița de pauză pentru a reflecta starea de redare
});
...
function updateCountdown() {
    const eventDate = new Date("June 14, 2025 00:00:00");
    ...
    // Dacă countdown-ul s-a încheiat
    if (remainingTime <= 0) {
        clearInterval(countdownInterval);
        document.querySelector('.count-down').innerHTML = "The event has started!";
    }
}
